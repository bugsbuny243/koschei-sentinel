...
`DefenseReflexCandidate` starts with `review_status = REVIEW_REQUIRED` and `training_authorization = false`.
...
An approved trajectory captures the range failure, why the prior interpretation or action was wrong, the correct Guard / Combat / Siege mode, the correct protected target and defensive action order, supporting evidence, post-action evidence, and immutable source/correction digests.
...
`sentinel-defense-reflex-build` consumes reviewed correction JSONL and emits `examples.jsonl` and `manifest.json`.
...
A raw world-model episode always has `training_authorization = false`. It cannot promote itself into training data. Each episode is bound to the exact source range report through `source_report_sha256`, and its digest includes the world-line timeline.
...
A causal example can be created only when the World Model Episode and reviewed correction belong to the same scenario, both reference the exact same source range-report SHA-256, the correction is APPROVED, its defensive outcome is verified, explicit training authorization is present, and the episode contains attack world-line lineage.

The resulting example binds temporal state history, graph changes, attacker reroutes, world-line observations, split/merge/reroute lineage, corrected interpretation and the verified defensive sequence into one provenance-preserving training item. `attack_world_line_sha256` is part of the training example digest, so lineage cannot be silently changed after review.

## Multi-incident Cyber Range
...
A Sentinel cyber training run is not considered ready merely because model weights and a dataset exist. The training bundle must bind four independent inputs: a ready Cyber Corpus collection-batch seal for the knowledge plane, a ready Defense Reflex Corpus manifest for immediate defensive behavior, a ready Causal Defense Corpus manifest for temporal/adversarial reasoning, and an independent evaluation-holdout SHA-256.
