...
def test_rank_coordinates_change_initialization_seed() -> None:
    spec = load_production_megatron_model_spec(CANDIDATE)
    ...
    left_q = next(item for item in left.tensors if item.name.endswith("self_attention.q_proj.weight"))
    right_q = next(item for item in right.tensors if item.name.endswith("self_attention.q_proj.weight"))
    assert left_q.seed != right_q.seed
