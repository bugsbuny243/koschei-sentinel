...
def test_preflight_fails_closed_without_persistent_bindings_and_paid_approval() -> None:
    ...
    result = cloud_runtime.preflight_cloud_runtime(profile, env={})
...
def test_profile_rejects_non_object_checkpoint_store() -> None:
    payload = cloud_runtime.load_cloud_runtime_profile(PROFILE).model_dump(mode="json")
    payload["stores"]["checkpoint_artifacts"]["kind"] = "CLICKHOUSE"

    try:
        cloud_runtime.CloudRuntimeProfile.model_validate(payload)
