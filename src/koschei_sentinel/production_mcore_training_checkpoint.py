...
class MCoreTrainingCheckpointResult(StrictModel):
    schema_version: Literal["sentinel.mcore-training-checkpoint.v1"] = (
        "sentinel.mcore-training-checkpoint.v1"
    )
...
def save_mcore_training_checkpoint(
    model: Any,
    optimizer: Any,
    training_state: MCoreTrainingState,
    *,
    checkpoint_dir: str | Path,
) -> MCoreTrainingCheckpointResult:
    try:
    ...
    return MCoreTrainingCheckpointResult(
        status="training_checkpoint_saved",
        checkpoint_dir=target.as_posix(),
        global_step=training_state.global_step,
    )


def load_mcore_training_checkpoint(
    model: Any,
    optimizer: Any,
    *,
    checkpoint_dir: str | Path,
    expected_global_seed: int,
) -> tuple[MCoreTrainingState, MCoreTrainingCheckpointResult]:
    try:
    ...
    template = _combined_sharded_state(
        model,
        optimizer,
    ...
    if missing or unexpected:
        raise ValueError(
            f"training checkpoint mismatch; missing={missing[:5]} unexpected={unexpected[:5]}"
        )
    ...
    training_state_raw = state.get("training_state")
    ...
    return training_state, MCoreTrainingCheckpointResult(
        status="training_checkpoint_loaded",
        checkpoint_dir=source.as_posix(),
        global_step=training_state.global_step,
    )
