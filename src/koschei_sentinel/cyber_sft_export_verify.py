...
def _invalid(*, violations: list[str], run_id: str | None = None) -> CyberSFTExportVerification:
...
def verify_cyber_sft_export(export_dir: str | Path) -> CyberSFTExportVerification:
    ...
    if missing:
        return _invalid(violations=["missing export files: " + ", ".join(sorted(missing))])
    ...
    attestation_verified = _attestation_digest(attestation_payload) == attestation.attestation_sha256
    ...
    try:
        ...
        receipt = CyberSFTTrainingReceipt.model_validate_json(required_files["receipt"].read_bytes())
        model_runtime = _json_object(required_files["model_runtime"], label="model-runtime.json")
        resume_runtime = _json_object(required_files["resume_runtime"], label="resume-runtime.json")
        corpus_manifest = _json_object(
            required_files["corpus_manifest"],
            label="defense-reflex-v3.manifest.json",
        )
    except (OSError, TypeError, ValueError) as exc:
        return _invalid(violations=[f"export artifact parsing failed: {exc}"], run_id=attestation.run_id)
    ...
    model_preflight_verified = (
        _sha256(required_files["model_preflight"]) == attestation.model_preflight_sha256
    )
    ...
    resume_runtime_verified = (
        _sha256(required_files["resume_runtime"]) == attestation.resume_runtime_sha256
    )
    ...
    if supplied_verification.model_dump(mode="json") != fresh_verification.model_dump(mode="json"):
        violations.append("exported verification report differs from fresh exported-run verification")
    ...
    examples_verified = _sha256(required_files["corpus_examples"]) == attestation.corpus_examples_sha256
    ...
    corpus_manifest_verified = (
        _sha256(required_files["corpus_manifest"]) == attestation.corpus_manifest_sha256
        and corpus_manifest.get("examples_sha256") == attestation.corpus_examples_sha256
    )
    ...
    repository_commit = required_files["repository_commit"].read_text(encoding="utf-8").strip()
